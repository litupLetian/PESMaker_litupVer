...
"""Structure generation for supercells, surfaces, defects, and perturbations."""
...
def format_generate_summary(
    ...
    *,
    ...
    for structure in result.structures:
        ...
        source_family_variants[(structure.task, structure.source, family)].add(
            structure.variant
        )
        variant_counts[
            (
    ...
    show_task_names = len(task_counts) > 1 or any(
        task != "default" for task in task_counts
    )
    ...
    *,
    ...
    for family in _variant_family_order():
        type_counts = {
            ...
            if count_task == task and count_source == source and count_family == family
        }
        if not type_counts:
            continue
